...
export function mapDecisionToResumeData(
  ...
  if (body.decision === 'reject') {
    const declinePatch = card?.decline?.argsPatch;
    if (declinePatch && typeof declinePatch === 'object') {
      return withNote({ decision: 'reject', ...declinePatch });
    }
    return withNote({ decision: 'reject' });
  }

  if (body.decision === 'modify') {
    const primaryPatch = card?.primary?.argsPatch;
    if (primaryPatch && typeof primaryPatch === 'object' && 'reportId' in primaryPatch) {
      return withNote({ decision: 'modify', ...primaryPatch });
    }
  ...
  if ('reportId' in primary) {
    return withNote({ decision: 'approve', ...primary });
  }
...
export function mountChatResumeRoute(app: Hono<AgentRouteEnv>, deps: AgentRouteDeps): void {
  app.post('/api/agent/v1/chat/resume', async (c) => {
    ...
    const card = ctx.proposedPayload as ApprovalCard | null;
    const toolId = typeof card?.meta?.toolId === 'string' ? card.meta.toolId : undefined;
