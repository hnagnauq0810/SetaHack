...
import { ALL_MANIFESTS } from '../../../src/shell/manifests.ts';
...
describe('visibleManifests', () => {
  it('keeps every registered manifest compatible with the shell nav contract', () => {
    expect(ALL_MANIFESTS.map((m) => [m.id, typeof m.useNavExtensions])).toEqual(
      ALL_MANIFESTS.map((m) => [m.id, 'function']),
    );
  });
